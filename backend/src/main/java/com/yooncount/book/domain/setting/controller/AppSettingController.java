...
public class AppSettingController {
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getFinnhubApiKey(
        ...
        boolean configured = service.get(principal.getId(), AppSettingService.KEY_FINNHUB_API_KEY).isPresent();
    ...
    public ResponseEntity<ApiResponse<Void>> setFinnhubApiKey(
            @AuthenticationPrincipal CustomUserPrincipal principal,
            @RequestBody Map<String, String> body) {
        String key = body.get("apiKey");
        if (key == null || key.isBlank()) {
            service.delete(principal.getId(), AppSettingService.KEY_FINNHUB_API_KEY);
        } else {
            service.set(principal.getId(), AppSettingService.KEY_FINNHUB_API_KEY, key.trim());
        }
